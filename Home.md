...
The test framework now automatically checks whether there is an `ON` condition for each `JOIN` clause in your Hive queries. Also, it checks whether each input view you provide in `basedOn` is declared as a dependency, creating a sanity check for dependency declarations in views.
