...
| `transforming` |  If a view has a `NoOp` transformation it immediately moves on to `no-data` state or `materialized`, depending on whether there is a `_SUCCESS_FLAG` in its `fullPath` or not. For other transformation types, tranformation is executed. Dependending on the result of the transformation, the view enters `materialized`, `retrying`, or `failed`.|
